...
export default function WastePosts() {
  const [posts, setPosts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortOrder, setSortOrder] = useState('newest'); // 'newest' | 'oldest'
  const [categoryFilter, setCategoryFilter] = useState('All'); // 'All' | 'Waste' | 'Announcement'
  ...
  const fetchPosts = async () => {
    try {
      setLoading(true);
      // Fetch both Waste and Announcement posts
      const [wasteRes, announcementRes] = await Promise.all([
        getFeedPostsApi('Waste'),
        getFeedPostsApi('Announcement')
      ]);

      const wasteList = wasteRes.data || [];
      const announcementList = announcementRes.data || [];

      // Combine and deduplicate
      const combined = [...wasteList, ...announcementList];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-5 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Publish official sanitation notices, schedules, or waste campaigns directly to the citizen social feed.
          </p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white hover:bg-emerald-700 font-bold rounded-xl transition-all shadow-md shadow-emerald-650/30 shrink-0 select-none hover:-translate-y-0.5 active:translate-y-0"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-605 flex items-center justify-center shrink-0 border border-emerald-100">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-1/3">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white bg-slate-50 transition-all font-medium text-slate-800"
          />
        </div>
        ...
        <div className="flex items-center gap-3 self-end md:self-center shrink-0">
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-bold">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg transition-all ${categoryFilter === 'All' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
            >
              All
            </button>
            <button
              onClick={() => setCategoryFilter('Announcement')}
              className={`px-3 py-1.5 rounded-lg transition-all ${categoryFilter === 'Announcement' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Announcement
            </button>
            <button
              onClick={() => setCategoryFilter('Waste')}
              className={`px-3 py-1.5 rounded-lg transition-all ${categoryFilter === 'Waste' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
            >
              Waste
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm">
          ...
          <p className="text-slate-500 text-sm mt-1 max-w-sm mx-auto font-semibold">
            {posts.length === 0
              ? 'Get started by creating your very first waste/sanitation announcement!'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sortedPosts.map((post) => {
            ...
            return (
              ...
              >
                <div className={`h-1.5 w-full absolute top-0 left-0 ${isAnnouncement ? 'bg-emerald-550' : 'bg-slate-400'}`} style={{ backgroundColor: isAnnouncement ? '#10b981' : '#64748b' }} />

                <div className="p-6 space-y-4">
                  <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                    <span className={`border rounded-full px-2.5 py-0.5 inline-flex items-center gap-1 font-extrabold ${isAnnouncement ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>
                      {post.category}
                    </span>
                ...
                <div className="border-t border-slate-100 px-6 py-4 flex items-center justify-between bg-slate-50/50 rounded-b-2xl">
                  <div className="flex items-center gap-2 text-xs text-slate-500 font-bold">
                    <span className="bg-slate-200 text-slate-700 px-2 py-0.5 rounded text-[10px]">
                      {post.author_name || 'Waste Department'}
                    </span>
                  </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl border border-slate-200/80 overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-emerald-50/20">
            ...
            <form onSubmit={handleCreatePost} className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-slate-50 font-semibold text-slate-800"
                >
                  <option value="Announcement">Announcement</option>
                  <option value="Waste">Waste Notice</option>
                </select>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  name="title"
                  required
                  placeholder="e.g. Sanitation drive scheduled for Ward 4 this Sunday"
                  value={newPostData.title}
                  onChange={handleInputChange}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-semibold text-slate-800 placeholder-slate-400 bg-slate-50 focus:bg-white"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-medium text-slate-800 placeholder-slate-400 bg-slate-50 focus:bg-white resize-none"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  name="location"
                  placeholder="e.g. Connaught Place, Ward 4"
              ...
              <div className="space-y-2">
                ...
                {imagePreview ? (
                  <div className="relative rounded-xl overflow-hidden border border-slate-200 max-h-52 bg-slate-100 flex items-center justify-center">
                    <img
                      src={imagePreview}
                      alt="Preview"
                  ...
                  <label className="border-2 border-dashed border-slate-200 hover:border-emerald-400 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all bg-slate-50 hover:bg-slate-50/20 group">
                    <ImageIcon className="w-8 h-8 text-slate-400 group-hover:text-emerald-500 transition-colors mb-2" />
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3 bg-white">
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition-all shadow-md shadow-emerald-650/20 select-none disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
