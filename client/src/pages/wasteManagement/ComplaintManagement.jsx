...
import { 
  ...
  LayoutGrid, List, AlertTriangle, HardHat, FileText, XCircle, ChevronDown, Image as ImageIcon, Trash2
} from 'lucide-react';
...
export default function ComplaintManagement() {
  ...
  useEffect(() => {
    const fetchAllWorkers = async () => {
      try {
        const res = await getWasteWorkers({ page: 1, page_size: 100 });
        setAllWorkers(res.data.items || (Array.isArray(res.data) ? res.data : []));
      } catch (err) {
  ...
  const getWorkerName = (id) => {
    ...
    if (worker) return worker.name || 'Sanitation Worker';
    return `Worker #${id}`;
  };
  ...
  const fetchComplaints = async () => {
    try {
      setLoading(true);
      const res = await getWasteComplaints({
        page,
        page_size: pageSize,
      ...
      setComplaints(items);
      setTotalItems(res.data.total || res.data.total_items || items.length);
    } catch (err) {
  ...
  const loadWorkers = async () => {
    try {
      const res = await getWasteWorkers({ page: 1, page_size: 100 });
      const items = res.data.items || (Array.isArray(res.data) ? res.data : []);
      setWorkers(items.filter(w => w.status === 'Active'));
    } catch (err) {
  ...
  const handleAssignWorker = async () => {
    ...
    try {
      setAssigning(true);
      const res = await assignWasteComplaintWorker(selectedComplaint.id, {
  ...
  const handleStatusChange = async (targetStatus) => {
    try {
      const res = await updateWasteComplaintStatus(selectedComplaint.id, {
        status: targetStatus,
  ...
  const handleConfirmAssignInline = async (complaintId) => {
    ...
    try {
      const res = await assignWasteComplaintWorker(complaintId, {
        assigned_worker_id: parseInt(selectedWorkerId),
        notes: "Assigned via waste complaints list."
      });
      toast.success("Worker assigned successfully!");
  ...
  const handleStatusChangeInline = async (complaintId, targetStatus) => {
    try {
      await updateWasteComplaintStatus(complaintId, {
        status: targetStatus,
        notes: targetStatus === 'RESOLVED' ? "Resolved by Department Admin" : undefined
      });
      toast.success(`Status updated to ${targetStatus}`);
  ...
  const handleDeleteComplaintDirect = async (complaintId) => {
    toastConfirm("Are you sure you want to delete this complaint record permanently?", async () => {
      try {
        await deleteWasteComplaint(complaintId);
  ...
  const getStatusColor = (currentStatus) => {
    if (currentStatus === 'NEW' || currentStatus === 'PENDING') return 'bg-emerald-600 shadow-emerald-600/40';
    if (currentStatus === 'ASSIGNED' || currentStatus === 'IN_PROGRESS') return 'bg-amber-500 shadow-amber-500/40';
    if (currentStatus === 'RESOLVED') return 'bg-teal-600 shadow-teal-600/40';
    ...
    <div className="space-y-6 relative h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Review, assign field crews, and monitor waste management complaints.</p>
        </div>
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        <form onSubmit={handleSearch} className="w-full md:w-1/3 flex gap-2">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all"
            />
          </div>
          <button type="submit" className="bg-[#064e3b] hover:bg-[#047857] text-white px-4 py-2 rounded-xl text-sm font-semibold transition-colors">
            Search
          </button>
        </form>
        ...
        <div className="w-full md:w-auto flex flex-wrap gap-3 justify-end">
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
        ...
        {viewMode === 'table' ? (
          ...
          <div className={`bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all ${selectedComplaint ? 'xl:col-span-2' : 'xl:col-span-3'}`}>
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-600 font-medium">
                <thead className="bg-slate-50 border-b border-slate-100 text-slate-455 uppercase text-xs font-semibold">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    ...
                    <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-bold text-white ${getStatusColor(c.status)}`}>
                          {c.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center flex items-center justify-center gap-1.5">
                        <button
                          ...
                          className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-xl transition-colors inline-flex items-center gap-1 text-xs font-bold whitespace-nowrap"
                        >
                        ...
                        <button
                          onClick={() => handleDeleteComplaintDirect(c.id)}
                          className="p-2 text-rose-600 hover:bg-rose-50 rounded-xl transition-colors inline-flex items-center gap-1 text-xs font-bold whitespace-nowrap"
                        >
                  ...
                  {!loading && complaints.length === 0 && (
                    <tr>
                      <td colSpan="6" className="text-center py-12 text-slate-400">No waste complaints match filters.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className={`space-y-6 transition-all ${selectedComplaint ? 'xl:col-span-2' : 'xl:col-span-3'}`}>
            <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
              <div>
                <h2 className="text-xl font-bold text-slate-900 font-sans tracking-tight">Active Waste Complaints</h2>
                <p className="text-xs text-slate-500 mt-0.5">Manage, assign sanitation workers, and review active waste complaints.</p>
              </div>
              <span className="px-3 py-1 text-xs font-bold rounded-full bg-emerald-50 text-emerald-700 border border-emerald-250">
              ...
              <>
                <div className="space-y-6">
                          ...
                          <div className="flex-grow space-y-4 w-full">
                            ...
                            {report.resolution_notes && (
                              <div className="bg-emerald-50/80 p-3.5 rounded-xl border border-emerald-200">
                                <span className="text-xs font-bold text-emerald-950 block mb-0.5">Resolution Notes:</span>
                            ...
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-t border-slate-100 pt-3">
                              <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-slate-500">
                                ...
                                {report.assigned_worker_id && (
                                  <span className="flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-100 font-bold">
                              ...
                              <div className="flex flex-wrap gap-2 items-center justify-end">
                                ...
                                <button
                                  ...
                                  className="bg-emerald-50 hover:bg-emerald-100 text-emerald-755 border border-emerald-200 text-xs font-bold py-2 px-3 rounded-lg shadow-sm transition-colors flex items-center gap-1.5"
                                >
                                  ...
                                  <>
                                    {assigningComplaintId === report.id ? (
                                      <div className="flex items-center gap-2">
                                        <select 
                                          className="border border-slate-355 rounded-lg text-sm p-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
                                ...
                                <button
                                  onClick={() => handleDeleteComplaintDirect(report.id)}
                                  className="bg-rose-50 hover:bg-rose-100 text-rose-755 border border-rose-200 text-xs font-bold py-2 px-3 rounded-lg shadow-sm transition-colors flex items-center gap-1.5"
                                >
                  ...
                  {complaints.length === 0 && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
                      ...
                      <p className="text-slate-500">There are no waste complaints reported at this time.</p>
                    </div>
                  )}
                </div>
                ...
                {totalItems > pageSize && (
                  <div className="flex items-center justify-between bg-white px-6 py-4 rounded-xl border border-slate-200 shadow-sm mt-6">
                    <div className="text-sm font-medium text-slate-500 font-sans">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-lg p-6 space-y-6 self-start xl:col-span-1">
            ...
            <div className="border-t border-slate-100 pt-5 space-y-4">
              ...
              <div className="space-y-3">
                <label className="text-xs text-slate-400 font-bold block">Select Available Sanitation Worker</label>
                <div className="flex gap-2">
                  <select
                    value={selectedWorkerId}
                    onClick={() => workers.length === 0 && loadWorkers()}
                    onChange={(e) => setSelectedWorkerId(e.target.value)}
                    className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-600 flex-grow focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  >
                    <option value="">-- Choose Worker --</option>
                    {workers.map(w => (
                      <option key={w.id} value={w.id}>{w.name} ({w.role || 'Cleaner'})</option>
                    ))}
                  </select>
                </div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white min-h-[60px] resize-none"
                />
