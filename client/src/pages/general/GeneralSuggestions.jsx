...
export default function GeneralSuggestions() {
  ...
  const fetchSuggestions = async () => {
    try {
      ...
      // Filter suggestions submitted for General department (category matching 'general')
      const filtered = (res.data || []).filter(
        (s) => (s.category || '').toLowerCase() === 'general'
      );
      setSuggestions(filtered);
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Failed to fetch general suggestions.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Review feedback, ideas, and improvement suggestions submitted by citizens for general services.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-1/3">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-slate-250 border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white bg-slate-50 transition-all font-medium"
          />
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center shadow-sm">
          ...
          <p className="text-slate-500 text-sm mt-1 max-w-sm mx-auto font-semibold">
            {suggestions.length === 0
              ? 'No citizen suggestions have been submitted to General Operations yet.'
            ...
            >
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider pt-2">
                  <span className="bg-amber-50 text-amber-700 border border-amber-100 rounded-full px-2.5 py-0.5 inline-flex items-center gap-1 font-extrabold">
